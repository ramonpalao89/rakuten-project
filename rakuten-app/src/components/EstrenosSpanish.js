...
import { retrieveEstrenosSpanish } from '../logic/retrieve-estrenos-spanish'
import { useDispatch, useSelector } from 'react-redux'
import { Link } from 'react-router-dom'

function EstrenosSpanish() {

    useEffect(() => {
        dispatch(retrieveEstrenosSpanish())
    }, [])

    const movieData = useSelector(state => state.spanishMovies.spanishMovies.data)
    const error = useSelector(state => state.spanishMovies.error)
...
export default EstrenosSpanish
