...
import { retrieveEstrenosTaquilla } from '../logic/retrieve-estrenos-taquilla'
import { useDispatch, useSelector } from 'react-redux'
import { Link } from 'react-router-dom'

function EstrenosTaquilla() {

    useEffect(() => {
        dispatch(retrieveEstrenosTaquilla())
    }, [])

    const movieData = useSelector(state => state.releasedMovies.releasedMovies.data)
    const error = useSelector(state => state.releasedMovies.error)
    ...
    return error ? (
        ...
        <div className='movies' style={{marginTop: '100px'}}>
...
export default EstrenosTaquilla
